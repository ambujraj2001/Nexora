...
const prettifySchema = z.object({
  user_query: z.string().describe('The original question or request from the user'),
  raw_data: z.string().describe('The raw result(s) returned by the previously executed tool(s)'),
});
...
export const prettifyResponseTool = tool(
  async ({ user_query, raw_data }: { user_query: string; raw_data: string }): Promise<string> => {
    log({ event: 'tool_execution_started', toolName: 'prettify_response', args: { user_query, raw_data } });
    ...
    const response = await llm.invoke([
      new SystemMessage(`
        ...
        - Be concise.
        - Do not add fluff.
        - Ensure the answer to the user's query is prominent.
      `),
  ...
  },
  {
    ...
    description: 'Call this to format raw data/numbers into a nice human-readable message before giving the final answer.',
